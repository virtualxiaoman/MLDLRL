...
2. 数据探索(EDA, Exploratory Data Analysis)与特征工程(FE, Feature Engineering)
3. 模型评估与选择
4. 模型训练与优化
...
1. **过采样**：增加少数类样本，如SMOTE算法。
2. **欠采样**：减少多数类样本，如Tomek Links算法，NearMiss算法。
3. **生成样本**：生成新样本，如GAN。
4. **评分加权**：对少数类样本加权。

如果对**R**有较高的要求，这时候几乎必须处理。如果评价指标是**AUC**，此时处理与不处理的差别不大。如果正负样本**同等**重要，可以不做处理。——来源：《机器学习算法竞赛实战》
...
2. **分层采样**：按照每个类别的比例采样。
3. **内存回收**：`gc.collect()`。
4. **数据类型优化**：`if df[col].min > np.iinfo(np.int8).min and df[col].max < np.iinfo(np.int8).max: df[col] = df[col].astype(np.int8)`。还可以将编码的ID(假设值很大但类别数量很少)转为从0开始。

> 每个对象都有一个引用计数器，记录有多少变量引用了它。当引用计数降为 0 时，内存会被自动释放。但如果两个或多个对象相互引用，引用计数永远不会为 0，导致内存泄漏。垃圾回收器`Garbage Collector`可以检测并回收这种情况。

#### 1.2.1.3 特征选择

假如特征数量很多，而仅有部分特征相关。如果使用Lasso，在存在相关特征时只会保留一个最重要的的特征，而将其他的相关特征设置为0，可能导致信息丢失。如果使用Ridge，并不会减少特征数量，不利于特征缩减。可以选择Elastic Net，它是L1和L2的结合，可以同时保留相关特征和减少特征数量。
`ElasticNet(alpha=lambda_value, l1_ratio=alpha)`
$$
J(\theta) = \frac{1}{2n} \sum_{i=1}^{n} \left( y^{(i)} - \hat{y}^{(i)} \right)^2 + \lambda \left( \alpha \sum_{j=1}^{D} |\theta_j| + \frac{1-\alpha}{2} \sum_{j=1}^{D} \theta_j^2 \right)
$$
其中$\alpha \in [0, 1]$是L1正则化和L2正则化的比例。当$\alpha = 1$时，Elastic Net等同于Lasso回归。
...
1. **XGBoost**：GBDT的改进版，二阶梯度提升算法，并行处理，同时分裂同一层的叶子(Level-wise按**层**生长)。
2. **LightGBM**：速度更快，每次从当前叶子中选择增益最大的节点进行分裂(Leaf-wise按**叶子**生长)，容易长出非常深的决策树而过拟合(可以调整`max_depth`)。
3. **CatBoost**：对类别特征进行处理(不要自作主张预处理)，自动处理缺失值，自动使用组合类别特征，可以使用GPU。(oblivious-tree，在树的每一层，所有节点的分裂特征和分裂阈值都是相同的)
...
> 机器学习在本质上还是特征工程，数据和特征决定了机器学习的上限，而模型和算法只是逼近这个上限而已。 ———— 吴恩达
...
2. **填充**：正态$\rightarrow$均值；偏态$\rightarrow$中位数；离散型/分类$\rightarrow$众数；分类$\rightarrow$新类别；有序数据$\rightarrow$next/previou。易产生有偏数据。
...
> **敏感性分析**：用相对改变量衡量结果对参数的敏感程度，$T$对$s$的敏感度$S(T,s)=\displaystyle\lim_{\Delta s \to 0}\frac{\Delta T/T}{\Delta s/s}=\frac{\partial T}{\partial s}\frac{s}{T}$。可以写为$f'(x) \frac{x}{f(x)}$，即$f(x)$在$x$处的相对变化率。
...
1. **可视化**：缺失值的可视化，如`sns.countplot(data=df, x='target', hue='missing')  # 查看在target下缺失值的分布`。
...
**为什么要离散化**：鲁棒性增强(平滑数据。降低噪声)、引入非线性关系、容易做特征交叉、降低计算复杂度。
...
对于对数变换，常使用$y=\ln(x+1)$，为了防止$x=0$，还能让变换后的是正值。

对于Box-Cox变换，当$\lambda=0$时，$y=\log(x)$；当$\lambda=0.5$时，$y=\sqrt{x}$；当$\lambda=1$时，$y=x$；当$\lambda=-1$时，$y=1/x$。注意$\lambda=0$准确公式是$y=\lim_{\lambda\to0}\frac{x^\lambda-1}{\lambda}$，对其洛必达即可，注意$x$是常数而$\lambda$才是变量。CBox-Cox是将Box-Cox中的$x$换成了$x+c$，这样可以处理负值。
...
<div style="display: flex; justify-content: center; align-items: center;">
    <div style="text-align: center;">
        <img src="assets/机器学习/images/image-9.png" style="width: 50%;"/>
...
<div style="display: flex; justify-content: center; align-items: center;">
    <div style="text-align: center;">
        <img src="assets/机器学习/images/image-10.png" style="width: 50%;"/>
...
4. 目标值编码：将类别特征`cat_feature`转换为目标变量`Target`的均值，但要注意防止数据泄露。
```python
kf = KFold(n_splits=5, shuffle=True, random_state=42)  # 五折交叉验证
for train_idx, val_idx in kf.split(train):
    train_fold, val_fold = train.iloc[train_idx], train.iloc[val_idx]  # 训练集和验证集划分
    target_mean = train_fold.groupby('cat_feature')['Target'].mean()  # 计算训练集中cat_feature的每个类别的目标均值
    train.loc[val_idx, 'cat_feature_TargetEncoded'] = val_fold['cat_feature'].map(target_mean)  # 用训练集的均值对验证集进行编码

global_target_mean = train.groupby('cat_feature')['Target'].mean()  # 计算整个训练集的目标均值，用于测试集编码
test['cat_feature_TargetEncoded'] = test['cat_feature'].map(global_target_mean)  # 用类别均值对测试集进行编码
overall_mean = train['Target'].mean()  # 处理测试集中未知类别（用训练集中目标值的全局均值填充）
test['cat_feature_TargetEncoded'].fillna(overall_mean, inplace=True)
```
5. count, nunique, ratio等统计量，可以代表用户点击次数、用户的兴趣宽度、用户偏好程度。


<p style="color:#EC407A; font-weight:bold">2. 特征交叉</p>

1. **显式交叉**，下面以$X=[x_1,x_2,x_3], Y=[y_1,y_2,y_3]$为例，先给出三种一般办法，再给出一些基于具体业务的方法：
...
- 数值之间的交叉：不妨设X是房屋面积，Y是房屋价格，那么$X\div Y$就是房屋的单位面积价格。
- 类别之间的交叉：不妨设X是用户的性别，Y是用户的年龄，那么$X\times Y$就是用户的性别年龄组合`性别_年龄`。
- 数值和类别之间的交叉：不妨设X是用户的年龄，Y是用户的性别，那么可以在性别的某个类别上计算年龄的统计量。
...
1. **时间戳**：将时间戳转换为年、月、日、时、分、秒等特征(分离维度)。
2. **时间差**：如用户两个行为之间的时间间隔。

### 2.2.4 多值特征处理

1. **展开**：将多值特征拆分为多个特征。`CountVectorizer`或者`df['tags_split'] = df['tags'].str.split(', ')`转为列表后使用`df_tags_dummies = df['tags_split'].str.join('|').str.get_dummies()`转为独热编码。
...
<p style="color:#EC407A; font-weight:bold">3. 互信息法</p>
...
<p style="color:#EC407A; font-weight:bold">6. null importance</p>

`permutation_importance`，先使用正确的特征和标签获得一个特征重要性分数，然后将标签打乱，再次计算特征重要性分数，如果前者没有大于后者，说明该特征是无用的。
...
待补充：https://andr-robot.github.io/%E6%9C%BA%E5%99%A8%E5%AD%A6%E4%B9%A0%E7%90%86%E8%AE%BA%E6%80%BB%E7%BB%93/
...
Curse of Dimensionality <mark>**维数灾难**</mark>：高维特征空间会让数据变得稀疏(Sparsity of Data)(稀疏是相当于空间的体积而言)，特征之间的相关性更强，计算复杂度增加，维度的可解释性降低。

在高维空间中，随机分布的数据点之间的**欧氏距离的相对差异会变得极小**，即所有点之间的距离趋于相等。高维空间中的数据点大多数分布在一个“超球”的表面，而非集中在内部。可以不准确地理解为小部分维度的数值的改变对距离的影响会被维度所稀释。该结论在[Wikipedia](https://en.wikipedia.org/wiki/Curse_of_dimensionality#Distance_function)有数学证明。
...
一般而言，默认$x$为输入，$y$为输出，$W_{n \times D}$为权重矩阵，$b_{n \times 1}$为偏置向量。向量约定为**列**向量。遇到shape不匹配的运算时默认使用**广播**。一般而言，在不影响阅读的情况下，向量$\mathbf{x}$或$\vec{x}$会简写为$x$。

不同于主流资料，在大部分情况下，本文将矩阵的shape记为$n \times D$而非$m \times n$，其中$n$是样本数，$D$是特征数。其中$D$指代dimension，这是因为：
1. $m \times n$的几乎没有实际意义，而且在线性代数的学习时向量组的$n$维向量其实是与线性方程组的$m$维向量的下标记号是矛盾的，很容易造成困惑。这里将所有矩阵统一为$n \times D$的形式，这样与线性代数中约定俗成的$n$维列向量一致。而且$n,D$无论是本身还是大小写区分度都比$m,n$高。当然，方阵记为$n \times n$。
2. 根据sklearn：In a data matrix, **features** are represented as **columns**: each column contains the result of applying a feature function to a set of samples. 也就是$D$其实表示的是`n_feature`，考虑到这会与`n_samples`重复，所以我的$D$取的特征向量的维度的含义。根据d2l: To avoid this confusion, we use **order** to refer to the number of axes(向量或轴的元素数量) and **dimensionality** exclusively to refer to the number of components(向量或轴的长度). 本文将dimension侧重于维度的数量，dimensionality侧重于维度概念本身。
...
线性回归是仿射变换(Affine Transformation，保持直线和平行性，但不一定保持欧几里德距离和角度)，相当于线性变换(linear transformation，加权和)+平移(translation，偏置项)。

> 仿射变换包括：平移translation, 缩放scaling, 位似变换homothety, 相似变换similarity, 镜像reflection, 旋转rotation, 双曲旋转hyperbolic rotation, 剪切shear mapping。
