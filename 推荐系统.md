...
召回的目的是筛选出尽可能有效的候选集，以减少后续精排的计算量，提高推荐效率。
...
$$P(w) = \frac{f(w)^{3/4}}{\sum\limits_{j = 0}^n f(w)^{3/4}}$$
...
$$\log\frac1{1+e^{-{v}_c^{T}{v}_{l_b}}} + \sum_{(l,m_n)\in\mathcal{D}_{m_n}}\log\frac1{1+e^{v_{m_n}^{T}{v}_l}}$$
...
+\sum_{(l,m_n)\in\mathcal{D}_{m_n}}\log\frac1{1+e^{v_{m_n}^{T}{v}_l}} \right)$$
...
**知识蒸馏**(knowledge distillation)：通过教师网络来指导学生网络的训练，将复杂、学习能力强的教师网络学到的特征表示“蒸馏”出来，传递给参数小、学习能力弱的学生网络，从而得到一个速度快、表达能力强的学生网络。教师网络可以是一个复杂的模型，甚至是集成模型，学生网络可以是一个简单的模型，参数数量、模型结构都可以不同。

教师网络的softmax可以改为：$y_i'=\frac{\exp{(y_i/T)}}{\sum_j\exp{(y_j/T)}}$，其中$T$是温度参数，$T$越大，输出分布越平滑。
学生网络不仅可以学习教师网络的输出，还可以学习教师网络的中间表示(隐藏层输出)。
...
> todo: 还可参考《机器学习高级实践·计算广告、供需预测、智能营销、动态定价》机械工业出版社 P136-152